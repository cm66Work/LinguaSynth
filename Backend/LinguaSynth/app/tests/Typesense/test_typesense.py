...
def documents():
  return [
    ...
    {'id': '8', 'name': 'doc_beta6', 'content': 'The ocean is vast and deep.'},
    {
      'id': '9',
      'name': 'doc_gamma',
      'content': 'Mountains are tall and majestic.',
    },
  ]
...
#     response = Manager.IndexDocuments(schema['name'], json.dumps(document))
...
#   response = Manager.RecreateCollection(
#     schema['name'], json.dumps(schema), True
#   )
#   assert response.Success  # will be true if all files are indexed correctly.
...
#   assert (
#     len(response.Data['documents']) == 1
#   )  # documents with the word sky in it.
...
#   assert (
#     len(response.Data['documents']) == 7
#   )  # documents with the word ocean in it.
